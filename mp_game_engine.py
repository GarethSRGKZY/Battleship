...
def generate_attack(board_size: int = 10, last_hit: Optional[Tuple[int, int]] = None, last_hit_direction: Optional[Tuple[int, int]] = None, previous_miss: Optional[Set[Tuple[int, int]]] = None, previous_hits: Optional[Set[Tuple[int, int]]] = None) -> Tuple[int, int]:
    """AI randomly guesses coordinates at first. If it hits a battleship, 
    it will try to hit the adjacent coordinates.
    ...
    Args:
        ...
        previous_hits (Optional[Set[Tuple[int, int]]], optional): Stores previous hits so that the AI will not hit it again. Defaults to None if missed.
    ...
    for direction in directions:
        ...
        if 0 <= next_x < board_size and 0 <= next_y < board_size and (next_x, next_y) not in previous_miss and (next_x, next_y) not in previous_hits:
            queue.append((next_x, next_y))

    while queue:
        ai_coord = queue.pop(0)
        if ai_coord not in previous_miss:
            return ai_coord
        
        for direction in directions:
            next_x, next_y = ai_coord[0] + direction[0], ai_coord[1] + direction[1]
            if 0 <= next_x < board_size and 0 <= next_y < board_size and (next_x, next_y) not in previous_miss and (next_x, next_y) not in previous_hits:
                queue.append((next_x, next_y))
...
def ai_opponent_game_loop() -> None:
    ...
    while any(size > 0 for size in user_ships.values()) and any(size > 0 for size in ai_ships.values()):
        ...
        print("\nAI Opponent's Turn:")
        ai_coordinates = generate_attack(board_size, last_hit, last_hit_direction, previous_miss, previous_hits)
        ai_result = attack(ai_coordinates, user_board, user_ships)
        if ai_result:
            previous_hits.add(ai_coordinates)
        else:
